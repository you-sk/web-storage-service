...
- **日時**: 2025-09-19
- **実装内容**:
  - ユーザープロフィール編集機能（ユーザー名・メールアドレスの変更）
  - パスワード変更機能（現在のパスワード確認＋新パスワード設定）
  - バックエンドAPIエンドポイントの追加（/api/users/profile, /api/users/change-password）
  - プロフィール画面の新規作成（編集フォーム・パスワード変更フォーム含む）
  - ナビゲーションにプロフィールリンクを追加
- **ブランチ**: feature/user-profile-password
- **次回予定**: ファイルの公開/非公開設定またはフォルダ機能の実装を推奨
